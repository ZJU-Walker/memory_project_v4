...
| 3 | `train_8gpu.sh` | `scripts/train.py pi05_yam_mem_v4_stage4e --fsdp-devices 8 --batch-size 16` in one process; auto-resume; log in `v4/diagnostics/` |
...
openpi/.venv/bin/python openpi/cluster_v4/coauthor/upload_checkpoint_to_hf.py --checkpoint v4/checkpoints/pi05_yam_mem_v4_stage4e/<EXP>/3000
```
...
```bash
bash openpi/cluster_v4/coauthor/download_checkpoint.sh <repo> v4/checkpoints/pi05_yam_mem_v4_stage4e/<EXP>/<step>
```
...
(`always` for Stage 4d/4e, `head` for Stage 4c).

What the run is: Stage 4e of the v4 recipe -- Pi0.5 base + a semantic fact bank (the fact
head grafted from the Stage-1 checkpoint and co-adapting under the Stage-1 abstention
objective) + a visual Titans bank, both banks allowed to commit on every valid step, 6000
updates, cosine LR 5e-5. Stage 4e supersedes Stage 4d (same forward pass; 4d let the read
loss reach the head through writes on non-evidence frames and its semantic read accuracy
collapsed to chance by update 1000). Health check on wandb: `diagnostic/v4_sem_commit_count`
should stay around 13-20 per update and `diagnostic/v4_fact_read_accuracy` near 1.0. The full
design, every stage's verdict and the evaluation batteries are documented in
`openpi/cluster_v4/README.md`.

Evaluating a checkpoint (development split, 8 held-out episodes):

```bash
cd openpi && source cluster_v35/env.sh
PYTHONPATH=scripts .venv/bin/python scripts/v4_side_flip_eval.py --config-name pi05_yam_mem_v4_stage4e \
    --params ../v4/checkpoints/pi05_yam_mem_v4_stage4e/<exp>/<step>/params --state-mask-prob 0 \
    --output-dir ../v4/diagnostics/side_flip_<exp>_<step>
PYTHONPATH=scripts .venv/bin/python scripts/v4_closed_loop_eval.py --config-name pi05_yam_mem_v4_stage4e \
    --params ../v4/checkpoints/pi05_yam_mem_v4_stage4e/<exp>/<step>/params --write-policy always \
...
pi05_yam_mem_v4_stage4e --write-policy always` and the client `examples/yam/client_memory_v4.py`
