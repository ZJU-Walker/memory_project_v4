...
# usage: run_banana0630_eval_inspect.sh stageA|stageB
...
source_root=$(mktemp -d "/tmp/v34-bani-${SLURM_JOB_ID}.${SLURM_STEP_ID}.XXXXXX")
...
if [[ "$mode" == stageA ]]; then
    A="--frame-mode inspect --frame-stride 1"
    O=v34_banana0630_writer_probe_inspect
    worker v34-bani-w1 0 "$O/500|$pilot/500|$art99/500/raw|$A" "$O/9500|$resume/9500|$art99/9500/raw|$A" <<< "$REMOTE" &
    worker v34-bani-w2 1 "$O/1000|$pilot/1000|$art99/1000/raw|$A" "$O/10000|$ckroot/10000|$art99/10000/raw|$A" "$O/15750|$ckroot/15750|$art74/15750/raw|$A" <<< "$REMOTE" &
    worker v34-bani-w3 2 "$O/2500|$pilot/2500|$art99/2500/raw|$A" "$O/11000|$resume/11000|$art99/11000/raw|$A" <<< "$REMOTE" &
    worker v34-bani-w4 3 "$O/5000|$ckroot/5000|$art99/5000/raw|$A" "$O/14750|$resume/14750|$art99/14750/raw|$A" <<< "$REMOTE" &
    wait
    echo "=== stageA done ==="
else
    B="--frame-mode full --frame-stride 1 --save-frame-scores"
    O=v34_banana0630_writer_probe_full15750
    worker v34-banf-w1 0 "$O|$ckroot/15750|$art74/15750/raw|$B --episodes 0,1,2,3,4,5,6,7" <<< "$REMOTE" &
    worker v34-banf-w2 1 "$O|$ckroot/15750|$art74/15750/raw|$B --episodes 8,9,10,11,12,13,14,15" <<< "$REMOTE" &
    worker v34-banf-w3 2 "$O|$ckroot/15750|$art74/15750/raw|$B --episodes 16,17,18,19,20,21,22" <<< "$REMOTE" &
    worker v34-banf-w4 3 "$O|$ckroot/15750|$art74/15750/raw|$B --episodes 23,24,25,26,27,28,29" <<< "$REMOTE" &
    wait
    echo "=== stageB done ==="
fi
