...
step="${1:?usage: $0 NUMERIC_CHECKPOINT_STEP}"
if [[ ! "$step" =~ ^[0-9]+$ ]]; then
...
output_root="$repo/diagnostic_outputs/v34_fixed_writer_probe/full_${job_id}/$step"
...
srun \
    ...
    --kill-on-bad-exit=1 \
    --job-name="v34-fixed-writer-${step}" \
    bash -s -- "$repo" "$dataset" "$checkpoint" "$snapshot" "$snapshot_sha" "$output_root" "$probe_uuid" <<'REMOTE'
set -euo pipefail
repo="$1"
dataset="$2"
checkpoint="$3"
...
source_root=$(mktemp -d "/tmp/v34-fixed-writer-${SLURM_JOB_ID}.${SLURM_STEP_ID}.XXXXXX")
cleanup_source() {
